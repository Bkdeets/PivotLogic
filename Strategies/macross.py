...
def sort_func(sma_obj):
    return sma_obj.sma[-1]
def rank(smas):
    return sorted(smas, key=sort_func)
def checkToSell(sma, price):
    if sma > price:
        return True
    return False
...
def get_orders(context, prices_df, position_size=200, max_positions=5):
    ...
    for symbol in symbols:
        sma = SMA(period, prices_df[symbol].dropna(), symbol)
        c += 1
        smas.append(sma)
    ...
    to_buy = [sma.ticker for sma in ranked[:max_positions]]
